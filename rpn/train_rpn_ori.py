...
from nets.rpn_training_opt import (RPNTrainer, get_lr_scheduler,
...
from utils.utils import (get_classes, seed_everything, show_config,
                         worker_init_fn)
from utils.utils_fit_rpn_opt import fit_one_epoch
...
if __name__ == "__main__":
    ...
    if True:
        ...
        train_util      = RPNTrainer(model_train, optimizer)
        #----------------------#
        #   记录eval的map曲线
        #----------------------#
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:
                ...
                #-------------------------------------------------------------------#
                nbs             = 16
                lr_limit_max    = 4e-4 if optimizer_type == 'adam' else 5e-2
                lr_limit_min    = 4e-6 if optimizer_type == 'adam' else 5e-4
                Init_lr_fit     = min(max(batch_size / nbs * Init_lr, lr_limit_min), lr_limit_max)
                Min_lr_fit      = min(max(batch_size / nbs * Min_lr, lr_limit_min * 1e-2), lr_limit_max * 1e-2)
                #---------------------------------------#
                #   获得学习率下降的公式
