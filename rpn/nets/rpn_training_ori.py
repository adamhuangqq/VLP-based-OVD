...
class AnchorTargetCreator(object):
    ...
    def __call__(self, bbox, anchor, gt_label):
        argmax_ious, label = self._create_label(anchor, bbox, gt_label)
        if (label > 0).any():
...
class RPNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        for i in range(n):
            ...
            # -------------------------------------------------- #
            gt_rpn_loc, gt_rpn_label    = self.anchor_target_creator(bbox, anchor[0].cpu().numpy(), label)
        ...
        a = 0.2    
        losses = [rpn_loc_loss_all/n, rpn_cls_loss_all/n]
