...
class AnchorTargetCreator(object):
    ...
    def _create_label(self, anchor, bbox):
        ...
        #-------------------------------------------------------#
        #   返回值：
        #   argmax_ious：每个先验框对应的最大的真实框的序号 
        #   label：每个先验框的标号，（-1，0，1）
        #-------------------------------------------------------#
...
class ProposalTargetCreator(object):
    ...
    def __call__(self, roi, bbox, label, roi_loc, roi_score, loc_normalize_std=(0.1, 0.1, 0.2, 0.2)):
        roi = np.concatenate((roi.detach().cpu().numpy(), bbox), axis=0)
        roi_loc = np.concatenate((roi_loc.detach().cpu().numpy(), np.zeros_like(bbox)), axis=0)
        roi_score = np.concatenate((roi_score.detach().cpu().numpy(), np.ones(len(bbox))), axis=0)
        # ----------------------------------------------------- #
        #   计算建议框和真实框的重合程度
        #   获得的iou的shape为[num_roi, num_gt]
        # ----------------------------------------------------- #
        ...
        #---------------------------------------------------------#
        #   sample_roi      [n_sample, ]
        #   gt_roi_loc      [n_sample, 4] 是四个调整值，不是坐标值
        #   gt_roi_label    [n_sample, ]
        #---------------------------------------------------------#
        keep_index = np.append(pos_index, neg_index)

        sample_roi = roi[keep_index]
        sample_roi_loc = roi_loc[keep_index]
        sample_roi_score = roi_score[keep_index]
        if len(bbox)==0:
            return sample_roi, sample_roi_loc, sample_roi_score, np.zeros_like(sample_roi), gt_roi_label[keep_index]
        ...
        return sample_roi, sample_roi_loc, sample_roi_score, gt_roi_loc, gt_roi_label
...
class RPNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        for i in range(n):
            ...
            #   gt_roi_label    [n_sample, ]
            # ------------------------------------------------------ #
            #这块做的事儿主要是对前面产生的框进行了采样，这个采样有两点，一个是减少建议框，第二是平衡正负样本
            #这个代码还做了一件事儿就是把前景标签打成具体的类别
            sample_roi, sample_roi_loc, sample_roi_score, gt_roi_loc, gt_roi_label = self.proposal_target_creator(roi, bbox, label, roi_loc, roi_score, self.loc_normalize_std)
            ...
            sample_roi_locs.append(torch.Tensor(sample_roi_loc).type_as(rpn_locs))
            sample_roi_scores.append(torch.Tensor(sample_roi_score).type_as(rpn_scores))
            ...
            gt_roi_label_bg = copy.deepcopy(gt_roi_label)
            gt_roi_label_bg[gt_roi_label_bg > 0] = 1
            
            sample_roi          = torch.Tensor(sample_roi).type_as(rpn_locs)
            sample_roi_loc      = torch.Tensor(sample_roi_loc).type_as(rpn_locs)
            sample_roi_score    = torch.Tensor(sample_roi_score).type_as(rpn_scores)
            gt_roi_loc          = torch.Tensor(gt_roi_loc).type_as(rpn_locs)
            gt_roi_label        = torch.Tensor(gt_roi_label).type_as(rpn_locs).long()
            gt_roi_label_bg     = torch.Tensor(gt_roi_label_bg).type_as(rpn_locs)

            #我觉得这部分的损失函数可以加大，这些选出来的框再做一个损失函数
            #print(sample_roi_score.size(),gt_roi_label_bg.size())
            roi_loc_loss_all += self._fast_rcnn_loc_loss(sample_roi_loc, gt_roi_loc, gt_roi_label, self.roi_sigma)
            roi_cls_loss_all += F.mse_loss(sample_roi_score, gt_roi_label_bg)
        ...
        losses = losses + [a * (losses[0] + losses[1]) + (1-a) * (losses[2] + losses[3])]
