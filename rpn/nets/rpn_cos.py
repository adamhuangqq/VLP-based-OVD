...
def center_point_list(input_size, stride):
    shape = (input_size[0]//stride, input_size[1]//stride)
    point_list = np.zeros((input_size[0]//stride, input_size[1]//stride, 2))
    for i in range(shape[0]):
        for j in range(shape[1]):
            point_list[i][j] = [i*stride + stride//2 , j*stride + stride//2]
    return point_list


def inornot(point, bbox , stride=16):
    num = 0
    for i, box in enumerate(bbox):
        left, top, right, bottom = box
        if (point[0] > left+stride//2) and (point[0] < right-stride//2) and (point[1] > top+stride//2) and (point[1] < bottom-stride//2) :
            num = i+1
            print(num)
            continue
    return num

def inornot_list(point_list, bbox):
    shape = point_list.shape[:2]
    mask = np.zeros(shape)
    for i in range(shape[0]):
        for j in range(shape[1]):
            mask[i][j] = inornot(point_list[i][j], bbox)
    return mask

# bbox = [[64, 64, 128, 128],[128, 128, 192, 192]]
# point_list = center_point_list([640,640], 16)
# print(inornot_list(point_list, bbox))




class AnchorTargetCreator(object):
    def __init__(self, stride=16, input_size=[600,600]):
        self.stride         = stride
    ...
    def _calc_centerness(self, bbox):
        #----------------------------------------------#
        #   anchor和bbox的iou，bbox是gt（左下右上）
        #   获得的ious的shape为[num_anchors, num_gt]
        #   即每个anchor都有gt个iou
        #----------------------------------------------#
        point_list = center_point_list(self.input_size, self.stride)
