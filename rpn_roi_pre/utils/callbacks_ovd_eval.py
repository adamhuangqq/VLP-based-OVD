...
class EvalCallback():
    def __init__(self, net, input_shape, class_names, num_classes, val_lines, log_dir, cuda, \
            map_out_path="map_out_eval", max_boxes=100, confidence=0.05, nms_iou=0.5, letterbox_image=True, MINOVERLAP=0.5, eval_flag=True, period=1):
        ...
        self.epoches    = [0]
        if self.eval_flag:
            with open(os.path.join(self.log_dir, "epoch_map.txt"), 'w') as f:
                f.write(str(0))
                f.write("\n")
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        for i, c in list(enumerate(top_label)):
            predicted_class = 'object'#self.class_names[int(c)]
            box             = top_boxes[i]
            ...
            for annotation_line in tqdm(self.val_lines):
                ...
                with open(os.path.join(self.map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
                    for box in gt_boxes:
                        ...
                        obj_name = 'object'#self.class_names[obj]
