...
class ProposalTargetCreator(object):
    def __init__(self, n_sample=128, pos_ratio=0.75, pos_iou_thresh=0.5, neg_iou_thresh_high=0.45, neg_iou_thresh_low=-1):
        self.n_sample = n_sample
        self.pos_ratio = pos_ratio
...
class FasterRCNNTrainer(nn.Module):
    ...
    def balanced_l1_loss(self, pred, target, gt_label,
                     beta=1.0,
                     alpha=0.5,
                     gamma=1.5,
                     reduction='mean'):
        pred        = pred[gt_label > 0]
        ...
        regression_loss = loss.sum()
        num_pos         = (gt_label > 0).sum().float()
        regression_loss /= torch.max(num_pos, torch.ones_like(num_pos))

        return regression_loss
    
    def iou_loss(self, pred_bbox, gt_bbox, gt_label, iou_mode='iou'):
        pred_bbox    = pred_bbox[gt_label > 0]
        gt_bbox      = gt_bbox[gt_label > 0]
        # print('11111111111')
        # print(pred_bbox)
        # print(gt_bbox)
        if iou_mode=='iou':
            iou = bbox_overlaps(pred_bbox, gt_bbox, is_aligned=True)
        #assert iou.all() > 0
        # print(iou.dtype)
        # print(iou)
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        for i in range(n):
            ...
            # print(gt_rpn_loc[0])
            # print( anchor[0][0])
            gt_rpn_bbox = loc2bbox(anchor[0], gt_rpn_loc)
            rpn_bbox = loc2bbox(anchor[0], rpn_loc)
            # -------------------------------------------------- #
            #   分别计算建议框网络的回归损失和分类损失
            # -------------------------------------------------- #
            rpn_loc_loss = self.iou_loss(rpn_bbox, gt_rpn_bbox, gt_rpn_label)
        ...
        for i in range(n):
            ...
            # -------------------------------------------------- #
            gt_roi_loc = gt_roi_loc*self.std
            roi_loc = roi_loc*self.std
            gt_roi_bbox = loc2bbox(sample_roi, gt_roi_loc)
            roi_bbox = loc2bbox(sample_roi, roi_loc)
            #roi_loc_loss = self.iou_loss(roi_bbox, gt_roi_bbox, gt_roi_label_bg)
            roi_loc_loss = self._fast_rcnn_loc_loss(roi_loc, gt_roi_loc, gt_roi_label.data, self.roi_sigma)
            roi_cls_loss = nn.CrossEntropyLoss()(roi_score, gt_roi_label_bg)
