...
CKimg_dir ='D:\\OVD\\datasets\\COCO_voc/images'
CKanno_dir = 'D:\\OVD\datasets\\COCO_voc/annotations'
...
def main():
    base_dir='D:/OVD/datasets/COCO_voc'#step1 这里是一个新的文件夹，存放转换后的图片和标注
    image_dir=os.path.join(base_dir,'images')#在上述文件夹中生成images，annotations两个子文件夹
    anno_dir=os.path.join(base_dir,'annotations')
    mkr(image_dir)
    mkr(anno_dir)
    origin_image_dir='D:/OVD/datasets/COCO/'#step 2原始的coco的图像存放位置
    origin_anno_dir='D:/OVD/datasets/COCO/ann'#step 3 原始的coco的标注存放位置
    verbose=False #是否需要看下标记是否正确的开关标记，若是true,就会把标记展示到图片上
    get_CK5(origin_anno_dir,origin_image_dir,verbose)
 
if __name__ == "__main__":
    #main()
    src_img = 'D:/OVD/datasets/COCO_voc/images/'
    dst_img = 'D:/OVD/datasets/COCO/val2017/'
    src_list = os.listdir(src_img)
    dst_list = os.listdir(dst_img)
    for i in dst_list:
